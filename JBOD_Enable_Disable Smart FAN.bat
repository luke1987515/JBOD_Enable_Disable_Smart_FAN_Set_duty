...
if "%Set%"=="1" set "operation=Enable"&set "operator=clear"
::Disable JBOD SamrtFAN, set 1:7:1
if "%Set%"=="2" set "operation=Disable"&set "operator=set"
:: Wrong Input
if not "%Set%"=="1" if not "%Set%"=="2" goto :END 

echo ====================
echo %operation% JBOD SamrtFAN
echo ====================
echo.
for /F %%i in ('sg_scan -s ^| find "AIC"') do (
  ...
  sg_ses --descriptor=!desc! --%operator%=1:7:1 %%i
  echo sg_ses --descriptor=!desc! --%operator%=1:7:1 %%i
  echo sg_ses --descriptor=!desc! --get=1:7:1 %%i
  :: sg_ses --descriptor=!desc! --get=1:7:1 %%i
